...
if($completeEvent == "true") {
	...
	if($Regs != NULL){
		...
		foreach ($arrayJoin as $SNumX) {
			...
			$getID = mysqli_fetch_assoc(mysqli_query($con,"SELECT DISTINCT Subject FROM TeachingInfo WHERE SNum = '$SNum' LIMIT 1")); // get prefix
	...
	while($row = mysqli_fetch_array($result))
	{
		...
		echo '<td>'; // Complete icons & complete submit rows
			if($row['Complete'] == NULL) {
				echo '<form method="post" action="events.php" onClick="this.submit();">
				<input type="hidden" name="completeEvent" type="text" value="true">
				<input type="hidden" name="id" type="text" value="'.$row['id'].'">';
				echo '<span class="glyphicon glyphicon-floppy-disk" style="cursor:pointer; font-size:1.5em; display: block; text-align:center; color:blue;"></span></form>';
			}
			else {
