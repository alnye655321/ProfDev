...
if (!empty($SNum)){
	$result = mysqli_query($con,"SELECT * FROM Activity WHERE SNum = '$SNum' AND VP IS NOT NULL");
		...
		$result = mysqli_query($con,"SELECT * FROM Activity WHERE LastName = '$LastName' AND  FirstName = '$FirstName' AND VP IS NOT NULL");
			...
			$result = mysqli_query($con,"SELECT * FROM Activity WHERE Prefix = '$dept' AND VP IS NOT NULL");
		}
...
echo '<form class="changeCursor1"><input type="hidden" name="levelOverride" type="text" value="true">
<input type="hidden" name="SNum" type="text" value="'.$row['SNumX'].'">
<span id="'.$checkID.'" style="font-size:1.5em; color:red;" class="glyphicon glyphicon-unchecked"></span></form>';
}

$admin = false;
if($admin == true){
$UnCheckImage = '<form class="changeCursor1"><input type="hidden" name="modalCheckLev3" type="text" value="true"><input type="hidden" name="SNum" type="text" value="'.$row['SNum'].'">
<span id="'.$checkID3.'" style="font-size:1.5em; color:red;" class="glyphicon glyphicon-unchecked"></span></form>';
}
...
// Table Body SQL Select
	while($row = mysqli_fetch_array($result))
	{
		...
		$EditImage2 = '<form class="changeCursor1"><input type="hidden" name="modalCheckLev2" type="text" value="true"><input type="hidden" name="SNum" type="text" value="'.$row['SNum'].'">
			<span id="'.$checkID2.'" style="font-size:1.5em; color:blue;" class="glyphicon glyphicon-edit"></span></form>';
		$EditImage3 = '<form class="changeCursor1"><input type="hidden" name="modalCheckLev3" type="text" value="true"><input type="hidden" name="SNum" type="text" value="'.$row['SNum'].'">
			<span id="'.$checkID3.'" style="font-size:1.5em; color:blue;" class="glyphicon glyphicon-edit"></span></form>';
		echo "<tr>";
...
// Table Body SQL Select
	while($row = mysqli_fetch_array($result))
	{
		...
		if(!empty($row['Payroll'])){$payrollCheck="checked";} else{$payrollCheck = "";}
		echo "<tr>";
		echo "<td>" . $row['SNum'] . "</td>";
		echo '<td>'.$row['LastName'].', '. $row['FirstName'].'</td>';
		echo "<td>" . $row['Type'] . "</td>";
		...
		echo "<td>" . $row['Hours'] . "</td>";
		echo '<td>
			<form class="form1" style="float:left">A: <input type="checkbox" '.$chairCheck.' name="Chair" value="'.$row['id'].'"><input type="hidden" name="pendingCheck" type="text" value="true"></form> 
			<form class="form1" style="float:right">D: <input type="checkbox" '.$chairCheck.' name="Chairdeny" value="'.$row['id'].'"><input type="hidden" name="pendingCheck" type="text" value="true"></form>  
		</td>';
		echo '<td>
			<form class="form1" style="float:left">A: <input type="checkbox" '.$VPCheck.' name="VP" value="'.$row['id'].'"><input type="hidden" name="pendingCheck" type="text" value="true"></form> 
			<form class="form1" style="float:right">D: <input type="checkbox" '.$VPCheck.' name="VPdeny" value="'.$row['id'].'"><input type="hidden" name="pendingCheck" type="text" value="true"></form>  
		</td>';
			...
			success: function (data) {
				console.log(data);
					if(data.type == 'ChairRec2Complete')	{
						...
						$('#' + SNumID).removeClass('glyphicon glyphicon-edit').addClass('glyphicon glyphicon-check').css('color','green');
						}
