...
if (!empty($SNum)){
	$result = mysqli_query($con,"SELECT * FROM Activity WHERE SNum = '$SNum' AND VP IS NOT NULL");
		...
		$result = mysqli_query($con,"SELECT * FROM Activity WHERE LastName = '$nameSearch' AND VP IS NOT NULL");
	}
...
// $EditImage = '<form class="changeCursor1" method="post" onClick="this.submit();" action="chair_rec.php"><input type="hidden" name="SNum" type="text" value="'.$row['SNum'].'">
// <span style="font-size:1.5em; color:blue;" class="glyphicon glyphicon-edit"></span></form>';

// Table Body SQL Select
	while($row = mysqli_fetch_array($result))
	{
		$EditImage = '<form class="changeCursor1"><input type="hidden" name="modalCheck" type="text" value="true"><input type="hidden" name="SNum" type="text" value="'.$row['SNum'].'">
			<span style="font-size:1.5em; color:blue;" class="glyphicon glyphicon-edit"></span></form>';
		echo "<tr>";
		...
		echo "<td style='text-align:center;'>"; if($row['ChairRec2'] == 1) {echo $CheckImage;} else {echo $EditImage;} echo"</td>";
		...
		echo "<td style='text-align:center;'>"; if($row['ChairRec3'] == 1) {echo $CheckImage;} else {echo $EditImage;} echo"</td>";
		echo "</tr>";
	}
...
if (!empty($SNum)){
	$result = mysqli_query($con,"SELECT * FROM Activity WHERE SNum = '$SNum' AND VP is NULL");
		...
		$result = mysqli_query($con,"SELECT * FROM Activity WHERE LastName = '$nameSearch' AND VP is NULL");
	}
