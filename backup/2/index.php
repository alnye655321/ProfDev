...
if (!empty($SNum)){
	$result = mysqli_query($con,"SELECT * FROM Activity WHERE SNum = '$SNum' AND Payroll IS NOT NULL");
}

	else if(!empty($nameSearch)) {
		$result = mysqli_query($con,"SELECT * FROM Activity WHERE LastName = '$nameSearch' AND Payroll IS NOT NULL");
	}
	
		else {
			$result = mysqli_query($con,"SELECT * FROM Activity WHERE Prefix = '$dept' AND Payroll IS NOT NULL");
		}
...
// Table Body SQL Select
	while($row = mysqli_fetch_array($result))
	{
		...
		echo '<td>
			<form class="form1" style="float:left">A: <input type="checkbox" '.$VPCheck.' name="VP" value="'.$row['id'].'"></form> 
			<form class="form1" style="float:right">D: <input type="checkbox" '.$VPCheck.' name="VPdeny" value="'.$row['id'].'"></form>  
		</td>';
