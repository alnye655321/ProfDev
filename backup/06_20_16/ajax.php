...
function levelUpdate($SNum)
{
	...
	$ChairRec2 = $getID["ChairRec2"]; $ChairRec3 = $getID["ChairRec3"];
	...
	if($level == 1){
		if($row_cnt >= 4 && ($ContactTotal >= 180 || $CreditsTotal >= 12) ){
			...
			if($EDU222 == 1 && $D2L == 1 && $NFO == 1 && $ChairRec2 == 1) {
				mysqli_query($con,"UPDATE Level SET PendingLevel = '2' WHERE SNum = '$SNum'"); //email HR?
			}
		}
	}
	...
	if($level == 2){
		if($row_cnt >= 6 && ($ContactTotal >= 360 || $CreditsTotal >= 24) ){
			...
			if($ChairRec3 == 1) {
				mysqli_query($con,"UPDATE Level SET PendingLevel = '3' WHERE SNum = '$SNum'"); //email HR?
			}		
		}
	}
