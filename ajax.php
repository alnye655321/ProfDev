...
function levelUpdate($SNum)
{
	...
	$getID = mysqli_fetch_assoc(mysqli_query($con3,"SELECT * FROM Level WHERE SNum = '$SNum'"));
	...
	$CreditsTotal = 0; $con3tactTotal = 0; //Calculate total contact hours and credit hours from COGNOS import TeachingInfo
	...
	$result1 = mysqli_query($con3,"SELECT Hours FROM Activity WHERE SNum = '$SNum' AND Type != 'D2L' AND Type != 'MPT' AND Type != 'NFO'");
		while($row = mysqli_fetch_array($result1))
		{
	...
	if($level == 1){
		if($row_cnt >= 4 && ($con3tactTotal >= 180 || $CreditsTotal >= 12) ){
			...
			if($EDU222 == 1 && $D2L == 1 && $NFO == 1 && $ChairRec2 == 1) {
				mysqli_query($con3,"UPDATE Level SET PendingLevel = '2' WHERE SNum = '$SNum'"); //update level display to show pending level change alert
	...
	if($level == 2){
		if($row_cnt >= 6 && ($con3tactTotal >= 360 || $CreditsTotal >= 24 && $hoursTotal >= 15) ){
			mysqli_query($con3,"UPDATE Level SET 6SemTeach = '1' WHERE SNum = '$SNum'");

			if($ChairRec3 == 1) {
				mysqli_query($con3,"UPDATE Level SET PendingLevel = '3' WHERE SNum = '$SNum'"); //update level display to show pending level change alert
...
function activeStatus($SNum) {
	global $con3;
	$getID = mysqli_fetch_assoc(mysqli_query($con3,"SELECT Inactive FROM Level WHERE SNum = '$SNum'"));
...
global $con3; global $id; global $con32;
...
//$getID = mysqli_fetch_assoc(mysqli_query($con3,"SELECT SNum FROM Activity WHERE id = '$id'"));
//$SNum = $getID["SNum"];

// lots of email here --->>>>>>>
if(!empty($Chair)){
	$id = @$_POST['Chair'];
	mysqli_query($con3,"UPDATE Activity SET Chair = '$today' WHERE id = '$id'");
	mysqli_query($con3,"UPDATE Activity SET Chairdeny = NULL WHERE id = '$id'");
	$return['type'] = "ChairdenyRemove";
	$return['msg'] = "ChairDenyID" . $id;

	//email
	$getID = mysqli_fetch_assoc(mysqli_query($con32,"SELECT Email FROM Users WHERE VP = '1' LIMIT 1"));
	$to = $getID["Email"];
...
if(!empty($VP)){
	...
	mysqli_query($con3,"UPDATE Activity SET VP = '$today' WHERE id = '$id'");
	mysqli_query($con3,"UPDATE Activity SET VPdeny = NULL WHERE id = '$id'");
	...
	if($devType == "D2L" || $devType == "MPT" || $devType == "NFO") {  //set Level table booleans if one of these types
		...
		mysqli_query($con3,"UPDATE Level SET $devType = '1' WHERE SNum = '$SNum'");
	}
...
if(!empty($Dean)){
	$id = @$_POST['Dean'];
	mysqli_query($con3,"UPDATE Activity SET Override = '1' WHERE id = '$id'"); // Override settings: 0 = proposed by chair; 1 = approved by dean
...
if(!empty($Chairdeny)) {
	...
	$id = @$_POST['Chairdeny'];
	mysqli_query($con3,"UPDATE Activity SET Chairdeny = '$today' WHERE id = '$id'");
}

if(!empty($VPdeny)) {
	//add column to activity table, deny, set deny to true & clear chair date $ email to chair/submitter
	$id = @$_POST['VPdeny'];
	mysqli_query($con3,"UPDATE Activity SET VPdeny = '$today' WHERE id = '$id'");
}
...
global $con3; global $id1;


 //Put form elements into post variables (this is where you would sanitize your data)
$id1 = @$_POST['id1'];

$result = mysqli_query($con3,"SELECT * FROM Activity WHERE id = '$id1'");
...
$result = mysqli_query($con3,"SELECT * FROM Level WHERE SNum = '$SNum'"); //check if all chair rec values are present
while($row = mysqli_fetch_array($result)) {
...
if($satTeach2 == "true") {mysqli_query($con3,"UPDATE Level SET ChairRec2Obs = '$today' WHERE SNum = '$SNum'");}
if($courseEval2 == "true") {mysqli_query($con3,"UPDATE Level SET ChairRec2Eval = '$today' WHERE SNum = '$SNum'");}
if($assReq2 == "true") {mysqli_query($con3,"UPDATE Level SET 	ChairRec2Ass = '$today' WHERE SNum = '$SNum'");}

if($satTeach3 == "true") {mysqli_query($con3,"UPDATE Level SET ChairRec3Obs = '$today' WHERE SNum = '$SNum'");}
if($courseEval3 == "true") {mysqli_query($con3,"UPDATE Level SET ChairRec3Eval = '$today' WHERE SNum = '$SNum'");}
if($assReq3 == "true") {mysqli_query($con3,"UPDATE Level SET 	ChairRec3Ass = '$today' WHERE SNum = '$SNum'");}
...
while($row = mysqli_fetch_array($result)) {
	...
	if($row['ChairRec2Obs'] != NULL && $row['ChairRec2Eval'] != NULL && $row['ChairRec2Ass'] != NULL) {
		mysqli_query($con3,"UPDATE Level SET ChairRec2 = '1' WHERE SNum = '$SNum'"); //then update overall ChairRec value based on level
		$return['type'] = 'ChairRec2Complete';
		$return['SNumID'] = $SNum . "ID2";
	}

	if($row['ChairRec3Obs'] != NULL && $row['ChairRec3Eval'] != NULL && $row['ChairRec3Ass'] != NULL) {
		mysqli_query($con3,"UPDATE Level SET ChairRec3 = '1' WHERE SNum = '$SNum'"); //then update overall ChairRec value based on level
...
global $con3; global $payrollVP; global $payrollHR;

if($payrollVP == "true") {
	$SNum = @$_POST['VPlevelIncrease'];
	mysqli_query($con3,"UPDATE Level SET levelIncreaseVP = '1' WHERE SNum = '$SNum'");
...
if($payrollHR == "true") {
	...
	mysqli_query($con3,"UPDATE Level SET Level = '$PendingLevel' WHERE SNum = '$SNum'");
	mysqli_query($con3,"UPDATE Level SET PendingLevel = NULL WHERE SNum = '$SNum'");
	mysqli_query($con3,"UPDATE Level SET levelIncreaseVP = NULL WHERE SNum = '$SNum'");
...
$result = mysqli_query($con3,"SELECT Inactive FROM Level WHERE SNum = '$SNum'"); //check if all chair rec values are present
while($row = mysqli_fetch_array($result)) {
...
if($Inactive == "true") {mysqli_query($con3,"UPDATE Level SET Inactive = '1' WHERE SNum = '$SNum'");}
else{mysqli_query($con3,"UPDATE Level SET Inactive = NULL WHERE SNum = '$SNum'");}
